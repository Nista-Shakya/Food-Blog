...
function fafe_get_field_names( $setting ) {
	$field_names = '';
...
function fafe_get_dynamic_preview( $setting ) {
...
function fafe_get_code_value( $setting, $post_id = false ) {
	if( ! $post_id ){
		if( is_author() ){
	...
	$dynamic_setting = preg_replace_callback( '/\[(.+?)?\]/', function( $matches ) use( $post_id ) {
		...
		if( $return_content[0] == 'user' || $return_content[0] == '$user' ){
			...
			$value = fafe_get_user_match_value( $return_content, $post_id );			
		}	
		if( $return_content[0] == 'post' || $return_content[0] == '$post' || $return_content[0] == 'product' ){
			$value = fafe_get_post_match_value( $return_content, $post_id );
		}	
		
		if( $return_content[0] == 'acf' || $return_content[0] == '$acf' ){
			$value = fafe_get_acf_match_value( $return_content, $post_id );
		}
		return $value;
	}, $setting );
	...
	function fafe_get_acf_match_value( $match_params, $post_id ){
		$return = '';
        ...
        switch( $field['type'] ){
            ...
            case 'user':
                ...
                if( $multiple && is_array( $value ) ){
                    ...
                    foreach( $value as $user ){
                        $return .= fafe_get_user_field( $term_field, $user );
                        if( $x < count($value) ) $return .= ', '; 
                        $x++;
                    }
                }else{
                    $return = fafe_get_user_field( $term_field, $value );
                }
                break;
            case 'post_object':
            case 'relationship':
            ...
            default:
                $return = $value;
				if( is_array( $return ) ){
					$return = fafe_implode_recur( ', ', $return );
				}
	...
	function fafe_get_user_match_value( $match_params, $post_id ){
		$value = '';
	...
	function fafe_get_post_match_value( $match_params, $post_id = false ){
		$value = '';
    ...
    function fafe_implode_recur($separator, $arrayvar) {
        $output = "";
        foreach ($arrayvar as $av)
        if (is_array ($av)) 
            $out .= fafe_implode_recur($separator, $av); // Recursive array 
        else                   
            $out .= $separator.$av;
    ...
    function fafe_get_user_field( $field, $user = null, $context = 'display' ) {
