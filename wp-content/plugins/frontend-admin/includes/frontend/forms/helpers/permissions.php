...
function fafe_can_edit_user( $user_id, $form_args ){
        ...
        if( isset( $user_id ) && is_user_logged_in() ){
            ...
            if( $active_user->ID == $user_id || get_user_meta( $user_id, 'fafe_manager', true ) == $active_user->ID ){
                $can_edit = $user_id;
            }
		}
		return $can_edit;
	}
function fafe_can_edit_post( $post_id, $form_args ){
    $active_user = wp_get_current_user();
    $can_edit = false;
