...
function fafe_get_elementor_instance() {
	return \Elementor\Plugin::$instance;
}

function fafe_get_current_post_id() {
	$el = fafe_get_elementor_instance();
...
function fafe_get_field_data( $type = null ){
	$field_types = [];
...
function fafe_get_group_field_keys( $field_group ){
...
function fafe_get_grouped_fields(){
	$fafe_field_groups = [];
	$field_groups = acf_get_field_groups();
	foreach($field_groups as $field_group){
		$fields = fafe_get_group_field_keys( $field_group );
		$fafe_field_groups[$field_group['key']] = $fields;
	}
	return $fafe_field_groups;
}

function fafe_get_user_id_fields(){
	$fields = fafe_get_acf_field_choices( false, 'user' );
	$keys = array_merge( ['[author]' => __( 'Post Author', 'frontend-admin' ) ],  $fields );
	return $keys;
}

function fafe_get_default_groups( $post_id ){
	$groups = [];
	global $post;
...
function fafe_get_user_roles( $exceptions = [], $all = false ){
...
function fafe_get_labeled_fields( $fields ){
	$field_choices = [];
	foreach( $fields as $field ){
...
function fafe_get_acf_field_group_choices(){
	$field_group_choices = [];
	$acf_field_groups = acf_get_field_groups();
	// loop through array and add to field 'choices'
	if( is_array( $acf_field_groups ) ) {        
		foreach( $acf_field_groups as $field_group ) {
			if( is_array( $field_group ) && ! isset( $field_group['fafe_group'] ) ){
...
function fafe_get_acf_field_choices( $groups = false, $type = false ){
...
function fafe_get_post_statuses_choices(){
...
function fafe_get_post_type_choices(){
	$post_type_choices = [];
	$args = array();
...
function fafe_get_image_folders(){
	$folders = ['all' => __( 'All Folders', 'frontend-admin' ) ];
...
function fafe_random_string($length = 15) {
...
function fafe_get_stripe_currencies(){
	return [
		'AFN' => 'Afghan Afghani',
		'ALL' => 'Albanian Lek',
		'DZD' => 'Algerian Dinar',
...
function fafe_get_paypal_currencies(){
	return [	
		'AUD' => 'Australian Dollar',
		'BRL' => 'Brazilian',
		'CAD' => 'Canadian Dollar',
...
function fafe_get_site_domain() {
	return str_ireplace( 'www.', '', parse_url( home_url(), PHP_URL_HOST ) );
}


function fafe_esc_attrs( $attrs ) {
	$html = '';
...
function fafe_slug_exists($post_name) {
    global $wpdb;
...
function fafe_parse_args( $args, $defaults ) {
	$new_args = (array) $defaults;

	foreach ( $args as $key => $value ) {
		if ( is_array( $value ) && isset( $new_args[ $key ] ) ) {
			$new_args[ $key ] = fafe_parse_args( $value, $new_args[ $key ] );
		}
		else {
			$new_args[ $key ] = $value;
		}
...
function fafe_edit_mode(){
	return \Elementor\Plugin::$instance->editor->is_edit_mode();	
}


function fafe_get_product_object(){
	if( isset( $GLOBALS['fafe_form'] ) ){
