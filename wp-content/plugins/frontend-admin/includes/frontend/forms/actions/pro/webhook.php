...
class SendWebhook extends ActionBase{
	...
	public function get_label() {
		return __( 'Webhook', 'frontend-admin' );
	}
	...
	public function register_settings_section( $widget ) {
        ...
        $repeater->add_control(
            'webhook_id',
           [
               'label' => __( 'Webhook Name', 'frontend-admin' ),
               'type' => Controls_Manager::TEXT,
               'placeholder' => __( 'Webhook Name', 'frontend-admin' ),
               'default' => __( 'Webhook Name', 'frontend-admin' ),
               'label_block' => true,
               'description' => __( 'Give this webhook an identifier', 'frontend-admin' ),
           ]
       );
		...
		$repeater->add_control(
			 'webhook_url',
			[
				'label' => __( 'Webhook URL', 'frontend-admin' ),
				'type' => Controls_Manager::TEXT,
				'placeholder' => __( 'Webhook URL', 'frontend-admin' ),
				'default' => __( 'Webhook URL', 'frontend-admin' ),
				'label_block' => true,
			]
		);
		...
		$widget->add_control(
			'webhooks_to_send',
			[
				'label' => __( 'Webhooks', 'frontend-admin' ),
			...
			foreach( $settings['emails_to_send'] as $email ){
				...
				if( $send_email ){
					...
					$fields = [
						...
						'email_reply_to' => 'noreplay@' . fafe_get_site_domain(),
						'email_reply_to_name' => '',
					...
					foreach ( $fields as $key => $default ) {
						...
						$setting = fafe_shortcode( $setting, $post_id );
						if ( ! empty( $setting ) ) {
							$fields[ $key ] = $setting;
						}
					}
					...
					do_action( 'fafe/mail_sent', $settings, $email, $fields, $step );
				}
			}
		}
