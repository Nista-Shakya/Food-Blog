...
(function($){
	...
	{
	file_input.parents('.hide-if-value').addClass('fafe-hidden').siblings('.show-if-value').addClass('show').find('img').attr('src',reader.result);
	}
	imagePreview = true;
	...
	$('body').on('click','a[data-name=remove]',function(e){
		if( typeof imagePreview != undefined ){
			$(this).parents('.show-if-value').removeClass('show').siblings('.hide-if-value').removeClass('fafe-hidden').find('input.image-preview').val('');
		}
	});   
	...
	$("body").on('change','.posts-select',function(a){
		var $select =$(this); 
		var $form =$select.parents('.fafe-form-posts').siblings('form');
		$select.parents('.fafe-form-posts').append('<span class="acf-loading"></span>');
		var widget = $form.data('widget');
		var formData = $form.find('input[name=_acf_form]').val();

		var ajaxData = {
			action:		'fafe/forms/change_form',
			draft: 		$(this).val(),
			form_data:	formData,
		};
		// get HTML
		$.ajax({
			...
			success: function(response){
				if(response.data.reload_form){
					...
					$form.siblings('.fafe-form-posts').remove();
	...
	$("body").on('click','span.close-msg',function(a){
		$(this).parents('.fafe-message').remove();
	});
	
	$("body").on('input click','.acf-fields,.acf-form-submit',function(a){$('.fafe-message').remove()});
	...
	$('body').on('click', 'button.edit-password', function(){
		$(this).addClass('fafe-hidden').siblings('.pass-strength-result').removeClass('fafe-hidden').parents('.acf-field-password').removeClass('edit_password').addClass('editing_password').next('.acf-field-password').removeClass('edit_password');
		$(this).after('<input type="hidden" name="edit_user_password" value="1"/>');
	});
	$('body').on('click', 'button.cancel-edit', function(){
		$(this).siblings('button.edit-password').removeClass('fafe-hidden').siblings('.pass-strength-result').addClass('fafe-hidden').parents('.acf-field-password').addClass('edit_password').removeClass('editing_password').next('.acf-field-password').addClass('edit_password');
	...
	function getForm( $el, $form_action ){
		var ajaxData = {
			action:		'fafe/fields/relationship/add_form',
	...
	$('body').on('click','form.fafe-form.-submit input[type=submit]',function(a){
	...
	$('body').on('submit','form.-delete', function (event) {
		...
		formData.append('action','fafe/delete_object');
		$.ajax({
			url: acf.get('ajaxurl'),
			type: 'post',
			data: formData,
	...
	$('body').on('submit','form.-submit', function (event) {
		...
		args = {
			...
			success: function ($form) {
				...
				var formData = new FormData($form[0]);          
				formData.append('action','fafe/form_submit');
				  ...
				  success: function(response){
					if(response.success) {	   				  
					  if( response.data.redirect ){
						...
						acf.unlockForm($form);	
						successMessage='<div class="fafe-message"><div class="acf-notice -success acf-success-message"><p class="success-msg">'+response.data.update_message+'</p><span class="fafe-dismiss close-msg acf-notice-dismiss acf-icon -cancel small"></span></div></div>';
						if(response.data.append){
						  ...
						  $form.find('.fafe-submit-button').attr('disabled',false).removeClass('acf-hidden');
						  if(response.data.clear_form){
							...
							if(response.data.update_message){
								...
								$('body, html').animate({scrollTop:$('.fafe-message').offset().top-50}, 'slow');
							}
						  }
						} 
					  }
	...
	$.each( dynamicValueFields, function( key, value ){
		...
		if( fieldSources.length > 0 ){
			...
			$.each( fieldSources, function( index, fieldName ){
				var fieldData = fafe_get_field_data(fieldName);               
				var sourceInput = fafe_get_field_element(fieldData[0], false);
				inputValue = fafe_get_field_input_value(inputValue, fieldData, sourceInput); 
				var sourceInput = fafe_get_field_element(fieldData[0], true);  
				sourceInput.on('input', function(){
					var returnValue = fieldDynamicValue;
					$.each( fieldSources, function( index, fieldName ){
					var fieldData = fafe_get_field_data(fieldName);               
					var sourceInput = fafe_get_field_element(fieldData[0], false);
					returnValue = fafe_get_field_input_value(returnValue, fieldData, sourceInput);
					});
			...
			function fafe_get_field_input_value(returnValue, fieldData, sourceInput){
				var shortcode = '['+fieldData[0]+']';
				if( sourceInput.val() != '' ){
				var display = sourceInput.val();
				if(fieldData[1] == 'text'){
					var display = fafe_get_field_text(fieldData[0]);
				}
			...
			function fafe_get_field_element(fieldName, all){
			...
			function fafe_get_field_data(fieldName){
			...
			function fafe_get_field_text(fieldName){
				var sourceField = fieldElement.siblings('div[data-name=' + fieldName + ']');
				if(sourceField.data('type') == 'radio'){
