...
class FAFE_Hide_Admin_Settings{
		/**
	...
	*/
	public function redirect_non_admin_users() {
		...
		if ( is_admin() && ! current_user_can( 'manage_options' ) && ! ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			$redirect_user = false;
			$hide_by_option = acf_decode_choices( get_option( 'fafe_hide_by' ) );
			$login_redirect = get_option( 'fafe_login_redirect' ); 
			...
			if( in_array( 'role', $hide_by_option ) ){
				...
				$redirect_roles = acf_decode_choices( get_option( 'fafe_roles' ) ); 
	...
	function hide_admin_area_option( $user ) {
		...
		echo '<h3>' . __( 'Hide WordPress Admin Area', 'frontend-admin' ) . '</h3>
			<table class="form-table">
				<tr>
					<th><label for="hide_admin_area">' . __( 'Hide Admin Area', 'frontend-admin' ) . '</label></th>
	...
	public function fafe_get_settings_fields( $field_keys ){
		$fafe_local_fields = array(
			array(
				'key' => 'fafe_hide_wp_dashboard',
				'label' => __( 'Hide WP Dashboard', 'frontend-admin' ),
				'name' => 'fafe_hide_wp_dashboard',
				'type' => 'true_false',
				'instructions' => '',
				'required' => 0,
				'conditional_logic' => 0,
			...
			array(
				...
				'only_front' => 0,
				'choices' => fafe_get_user_roles( ['administrator'] ),
				'allow_null' => 0,
				'multiple' => 1,
				'ui' => 1,
				'ajax' => 0,
			...
			array(
				'key' => 'fafe_login_redirect',
				'label' => __( 'Redirect to', 'frontend-admin' ),
				'name' => 'fafe_login_redirect',
				'type' => 'url',
	...
	public function __construct() {
		...
		add_filter( 'fafe/hide_admin_fields', [ $this, 'fafe_get_settings_fields'] );
